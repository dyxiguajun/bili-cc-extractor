const I18N = {
  zh: {
    ...
    loginMenu: "用户菜单",
    shutdownApp: "关闭程序",
    shutdownConfirm: "关闭程序后，本地服务会停止运行，当前浏览器页面将无法继续使用。确定关闭吗？",
    shuttingDown: "正在关闭程序...",
    shutdownDone: "程序已关闭，正在关闭当前页面。",
    shutdownFail: "关闭程序失败，请稍后重试或手动关闭。"
  },
  en: {
    ...
    loginMenu: "User menu",
    shutdownApp: "Shut down app",
    shutdownConfirm: "This will stop the local backend service. The current browser page will no longer work. Continue?",
    shuttingDown: "Shutting down...",
    shutdownDone: "The app has been shut down. Closing this page.",
    shutdownFail: "Failed to shut down the app. Please try again or stop it manually."
  }
};
...
const menuShutdownBtn = $("menuShutdownBtn");
...
async function shutdownApp() {
  ...
  setStatus(t("shuttingDown"));
  try {
    await fetch("/api/shutdown", { method: "POST", cache: "no-store", keepalive: true });
    setStatus(t("shutdownDone"));
    btn.disabled = true;
    loginBtn.disabled = true;
    copyBtn.disabled = true;
    downloadBtn.disabled = true;

    const closingText = t("shutdownDone");
    document.body.innerHTML = `
      <div class="shutdown-page">
        <div class="shutdown-card">
          <h1>Bili CC Extractor</h1>
          <p>${closingText}</p>
          <p class="muted">${currentLang === "zh" ? "如果浏览器没有自动关闭，可以手动关闭这个标签页。" : "If the browser does not close automatically, you can close this tab manually."}</p>
        </div>
      </div>
    `;
...
menuShutdownBtn.addEventListener("click", shutdownApp);
